...
if ($_GET['langer'] == 'install' or $_GET['langer'] == 'installnew') {
	...
	if ($chmod_check == false) {
		...
		if (defined('EASYPOPULATE_MSGSTACK_LANGER') && strpos(EASYPOPULATE_MSGSTACK_LANGER, 'paypal@portability.com.au') == true) {
			...
			$messageStack->add('批量商品管理作者: <b>langer</b>. 捐款帐号: paypal@portability.com.au', 'caution');
		}
...
?>
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						<td class="pageHeading"><?php echo "批量商品管理 $curver"; ?></td>
					</tr>
				</table>
<?php
				echo zen_draw_separator('pixel_trans.gif', '1', '10');
							...
							<table width="70%" border="0" cellpadding="8" valign="top">
								<tr>
									<td width="100%">
										<p>
											<form ENCTYPE="multipart/form-data" ACTION="easypopulate.php" METHOD="POST">
												<div align = "left">
													...
													<input type="submit" name="buttoninsert" value="导入数据库">
													<br />
												</div>
											</form>
											<br>
											<form ENCTYPE="multipart/form-data" ACTION="easypopulate.php" METHOD="POST">
												<div align = "left">
													<b>从临时目录导入 (<?php echo $tempdir; ?>)</b><br />
										...
										<a href="easypopulate.php?download=stream&dltype=full">下载<b>完整的</b>.csv文件</a>
<?php if ($products_with_attributes == true) { ?>
										<span class="fieldRequired"> (包含属性)</span>
<?php } else { ?>
										<span class="fieldRequired"> (不含属性)</span>
<?php } ?>
										<br />
										<a href="easypopulate.php?download=stream&dltype=priceqty">下载 <b>型号/价格/数量</b> 的.csv文件</a><br />
										<a href="easypopulate.php?download=stream&dltype=modqty">下载 <b>型号/价格/数量/最后更新/状态</b> 的.csv文件</a><br />
										<a href="easypopulate.php?download=stream&dltype=category">下载 <b>型号/分类</b> 的.csv文件</a><br />
										<a href="easypopulate.php?download=stream&dltype=froogle">下载 <b>Froogle</b> 的Tab分割的.txt文件</a><br />
										<a href="easypopulate.php?download=stream&dltype=attrib">下载 <b>型号/属性</b> 的.csv文件</a>
										<br /><br />
										<b>在临时目录中创建批量商品和Froogle文件: <a href="../<?php echo $tempdir; ?>"><?php echo $tempdir; ?></a></b>
										<br /><br />
										<a href="easypopulate.php?download=tempfile&dltype=full">在临时目录中创建 <b>完整的</b> .txt文件</a>
<?php if ($products_with_attributes == true) { ?>
										<span class="fieldRequired"> (包含属性)</span>
<?php } else { ?>
										<span class="fieldRequired"> (不含属性)</span>
<?php } ?>
										<br />
										<a href="easypopulate.php?download=tempfile&dltype=priceqty">在临时目录中创建 <b>型号/价格/数量</b> 的.csv文件</a><br />
										<a href="easypopulate.php?download=tempfile&dltype=modqty">在临时目录中创建 <b>型号/价格/数量/最后更新/状态</b> 的.csv文件</a><br />
										<a href="easypopulate.php?download=tempfile&dltype=category">在临时目录中创建 <b>型号/分类</b> 的.csv文件</a><br />
										<a href="easypopulate.php?download=tempfile&dltype=froogle">在临时目录中创建 <b>Froogle</b> 的Tab分割的.txt文件</a><br />
										<a href="easypopulate.php?download=tempfile&dltype=attrib">在临时目录中创建 <b>型号/属性</b> 的.csv文件</a><br />
									</td>
								</tr>
							</table>
<?php
